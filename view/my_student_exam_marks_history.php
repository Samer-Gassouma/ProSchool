...
$sql="SELECT * FROM class, subject_routing,teacher WHERE class.id=subject_routing.class_id   AND subject_routing.teacher_id=teacher.id AND teacher.index_number='$index' ";
$result=mysqli_query($conn,$sql);
if(mysqli_num_rows($result) > 0){
	while($row=mysqli_fetch_assoc($result)){
?> 
...
<?php }} ?>
                  	...
                  	<div class="box-footer">
                    	...
                    	<button type="button" id="btnSubmit" class="btn btn-primary"  onClick="showMyStudent(this)">Submit</button>
                  	</div>
            	</div>
        	</div>
    	</div>
