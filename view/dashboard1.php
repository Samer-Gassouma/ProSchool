...
<div class="content-wrapper">
    ...
    <?php
		$check = "SELECT * FROM student WHERE index_number='$my_index' and color ='red' AND Id_school = '$my_school'";
		$result = mysqli_query($conn,$check);
		if(mysqli_num_rows($result)>0){
			echo'<div class="alert alert-warning alert-dismissible fade in">
			<strong>Warning!</strong> You are Absent for today. contact your Adminastrator.
		  </div>';
		}
		?>
        <?php
			$check = "SELECT * FROM subject_routing WHERE class_id='$class_id' AND Id_school = '$my_school'";
			$result = mysqli_query($conn,$check);
			if($result){
				while($row = mysqli_fetch_assoc($result)){
					$tid = $row['teacher_id'];
					$x = "SELECT * FROM teacher where id = $tid and color='red' ";
					$result1 = mysqli_query($conn,$x);
					if($result1){
						while($row11 = mysqli_fetch_assoc($result1)){ 
							$fname = $row11["full_name"];
							echo'<div class="alert alert-danger alert-dismissible fade in">
								<strong>Warning!</strong> You are Teacher  '; echo($fname);  echo'  is Absent for today.
							</div>';
						}
					}
				}
     ...
     <div class="row" id="table1">
        	<div class="col-md-8">
           		<center><h4 class="box-title">Home Work</h4></center>
				   <?php 
					include_once('../controller/config.php');
					$sql="SELECT * FROM homework_upload WHERE id_class = $class_id AND Id_school = $my_school";
					$result=mysqli_query($conn,$sql);
					if(mysqli_num_rows($result)>0){
						while ($rowx=mysqli_fetch_assoc($result)){  
							$teacher_id=$rowx['id_Teacher'];
							$sql1="SELECT * FROM teacher WHERE id = $teacher_id AND Id_school = $my_school";
							$result1=mysqli_query($conn,$sql1);
							$row1=mysqli_fetch_assoc($result1);
							$teacher_name=$row1['full_name'];
							$image=$row1['image_name'];
							?>

							<div style="   border: 1px solid; padding: 5px; box-shadow: 5px 10px #888888; padding-top:20px; padding-left:20px;" class="media">
							...
							<p><?php echo $rowx['Comment']; ?></p>
							<labele>Date:</labele> <?php echo $rowx['Date_homework']; ?> <br>
							<?php if ($rowx['File_name'] !=""){ ?>
					...
					<hr>
						<?php } } else{
							echo "<center><h4 class='media-heading'>No HomeWork for Today! Lucky You </h4></center>" ; } ?>
...
for($i=0; $i<count($month); $i++){
	$sql="SELECT COUNT(id) FROM student_attendance WHERE year='$current_year' AND month='$month[$i]' AND index_number='$my_index' AND Id_school = $my_school ";
	$result=mysqli_query($conn,$sql);
...
$sql2="select  subject_routing.class_id as g_id,subject_routing.subject_id as s_id,subject_routing.teacher_id as t_id
       from student_subject
       inner join subject_routing
       on student_subject.sr_id=subject_routing.id
       where student_subject.index_number='$index' and student_subject.year='$current_year' AND student_subject.Id_school = '$my_school'";
...
while($row2=mysqli_fetch_assoc($result2)){
    ...
    $sql3="SELECT 
           ...
           FROM
              timetable
           WHERE
              class.id='$g_id' AND subject_id='$s_id' AND teacher_id='$t_id'  AND Id_school = $my_school
           ORDER BY
              start_time";
...
$sql="select subject.name as s_name,teacher.full_name as t_name,class_room.name as c_name
      from timetable
      inner join subject
      ...
      where  timetable.class='$g_id' AND timetable.Id_school = $my_school and timetable.subject_id='$s_id' and timetable.teacher_id='$t_id' and timetable.start_time='$s_time' and timetable.end_time='$e_time' and timetable.day='Sunday'";
...
$sql="select subject.name as s_name,teacher.full_name as t_name,class_room.name as c_name
      ...
      inner join class_room
      on timetable.classroom_id=class_room.id
      where timetable.class='$g_id' AND timetable.Id_school = $my_school and timetable.subject_id='$s_id' and timetable.teacher_id='$t_id' and timetable.start_time='$s_time' and timetable.end_time='$e_time' and timetable.day='Monday'";
...
$sql="select subject.name as s_name,teacher.full_name as t_name,class_room.name as c_name
      ...
      inner join class_room
      on timetable.classroom_id=class_room.id
      where timetable.class='$g_id' and timetable.Id_school = $my_school AND timetable.subject_id='$s_id' and timetable.teacher_id='$t_id' and timetable.start_time='$s_time' and timetable.end_time='$e_time' and timetable.day='Tuesday'";
...
$sql="select subject.name as s_name,teacher.full_name as t_name,class_room.name as c_name
      ...
      inner join class_room
      on timetable.classroom_id=class_room.id
      where timetable.class='$g_id' AND timetable.Id_school = $my_school and timetable.subject_id='$s_id' and timetable.teacher_id='$t_id' and timetable.start_time='$s_time' and timetable.end_time='$e_time' and timetable.day='Wednesday'";
...
$sql="select subject.name as s_name,teacher.full_name as t_name,class_room.name as c_name
      ...
      inner join class_room
      on timetable.classroom_id=class_room.id
      where timetable.class='$g_id' AND timetable.Id_school = $my_school and  timetable.subject_id='$s_id' and timetable.teacher_id='$t_id' and timetable.start_time='$s_time' and timetable.end_time='$e_time' and timetable.day='Thursday'";
...
$sql="select subject.name as s_name,teacher.full_name as t_name,class_room.name as c_name
      ...
      inner join class_room
      on timetable.classroom_id=class_room.id
      where timetable.class='$g_id' AND timetable.Id_school = $my_school and timetable.subject_id='$s_id' and timetable.teacher_id='$t_id' and timetable.start_time='$s_time' and timetable.end_time='$e_time' and timetable.day='Friday'";
...
$sql="select subject.name as s_name,teacher.full_name as t_name,class_room.name as c_name
      ...
      inner join class_room
      on timetable.classroom_id=class_room.id
      where timetable.class='$g_id' AND timetable.Id_school = $my_school and timetable.subject_id='$s_id' and timetable.teacher_id='$t_id' and timetable.start_time='$s_time' and timetable.end_time='$e_time' and timetable.day='Saturday'";
