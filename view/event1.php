...
if($my_type == 'Teacher' ){
	...
	$sqls = "SELECT * FROM student WHERE std_index=$my_index";
	$results = mysqli_query($conn,$sqls);
	$rows = mysqli_fetch_assoc($results);
	$Id_School =$rows['Id_School'];
}else{ 
...
$sql="SELECT * FROM events WHERE create_by='$my_index' and creator_type='$my_type' and year='$year' and month='$month' and Id_School ='$Id_School'";
