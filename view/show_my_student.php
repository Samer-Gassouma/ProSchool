...
$sql1="SELECT * FROM subject_routing WHERE teacher_id='$teacher_id' AND	class_id='$grade_id'";
$result1=mysqli_query($conn,$sql1);

while($row1=mysqli_fetch_assoc($result1)){
	$sr_id=$row1['id'];
	
	$sql2="select * FROM student WHERE class_id='$grade_id'";
	...
	if(mysqli_num_rows($result2) > 0){
		...
		$count++;
		$std_index=$row2["index_number"];
?>   
          			<tr>
                		<td><?php echo $count; ?></td>
                    	<td id="td1_<?php echo $row2['id']; ?>">
                    		<a href="#modalviewform" onClick="studentProfile('<?php echo $row2["id"]; ?>')" class=""  data-toggle="modal"><?php echo $row2['full_name']; ?></a>
                    	</td>
                    	<td> 
<?php
$sql3="SELECT * FROM student_exam WHERE index_number='$std_index' AND class_id='$grade_id' AND exam_id='$exam_id' AND year='$year'";
