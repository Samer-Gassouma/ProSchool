					...
					<div class="card-body">
						<table class="table table-condensed table-bordered table-hover">
							...
							<tbody>
								<?php 
									$i = 1;
									$sql = "SELECT * FROM student_ef_list order by id asc ";
									$fees = $conn->query($sql);
									while($row=$fees->fetch_assoc()):
										$user_id = $row['student_id'];
										$query = $conn->query("SELECT * FROM student where id = '$user_id' ");
										$row1 = $query->fetch_array();
										
										$paid = $conn->query("SELECT sum(amount) as paid FROM payments where ef_id=".$user_id);
								...
								<tr>
									...
									<td>
										<p> <b><?php echo ucwords($row1['full_name']) ?></b></p>
									</td>
									<td class="text-right">
